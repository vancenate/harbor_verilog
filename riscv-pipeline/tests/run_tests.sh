...
if [ ! -d "$SUITES_DIR" ] || [ ! -d "$IMEM_DMEM" ]; then
    echo "ERROR: suites/ or imem_dmem/ not found. Run from riscv-pipeline/tests/"
    exit 1
fi
# For local runs, copy design files from solution if not present (Harbor copies from /workspace)
if [ ! -f "$TESTS_DIR/pipeline.v" ]; then
    SOLUTION="$TESTS_DIR/../solution"
    if [ -f "$SOLUTION/pipeline.v" ]; then
        cp "$SOLUTION/pipeline.v" "$TESTS_DIR/"
        for f in "$SOLUTION"/*.v "$SOLUTION"/*.vh; do
            [ -f "$f" ] && [ "$(basename "$f")" != "pipeline.v" ] && cp "$f" "$TESTS_DIR/"
        done
    else
        echo "ERROR: pipeline.v not found. Copy pipeline RTL into tests/ or run from repo with solution/."
        exit 1
    fi
fi
