...
# Reference solution for the 24-bit Carry Select Adder task
...
module solution_core (
    ...
    genvar i;
    generate
        for (i = 0; i < NSTAGE; i++) begin : stage
            if (i == 0) begin
                rca #(.DATA_WIDTH(STAGE_WIDTH)) rca_cin0 (
                    .a(a[7:0]),
                    .b(b[7:0]),
                    .cin(1'b0),
                    .sum({cout_0[0], psum_0[0]})
                );
            end else begin
                rca #(.DATA_WIDTH(STAGE_WIDTH)) rca_cin0 (
                    .a(a[STAGE_WIDTH*(i+1)-1:(STAGE_WIDTH*i)]),
                    .b(b[STAGE_WIDTH*(i+1)-1:(STAGE_WIDTH*i)]),
                    .cin(1'b0),
                    .sum({cout_0[i], psum_0[i]})
                );
                rca #(.DATA_WIDTH(STAGE_WIDTH)) rca_cin1 (
                    .a(a[STAGE_WIDTH*(i+1)-1:(STAGE_WIDTH*i)]),
                    .b(b[STAGE_WIDTH*(i+1)-1:(STAGE_WIDTH*i)]),
                    .cin(1'b1),
                    .sum({cout_1[i], psum_1[i]})
                );
            end
        end
    endgenerate
    ...
    generate
        for (i = 0; i < NSTAGE; i++) begin : mux_sum
            if (i == 0) begin
    ...
    generate
        for (i = 0; i < NSTAGE; i++) begin : mux_cout
            if (i == 0) begin
